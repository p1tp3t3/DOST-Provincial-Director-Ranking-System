...
import { ISLANDS, PROVINCE_REGIONS } from '@/Data/provinceGeography';
...
// ── Top-1 director per region ─────────────────────────────────────────────
const topPerRegion = computed(() => {
    ...
    for (const r of allRows) {
        ...
        if (selectedIsland.value !== 'all') {
            const pInfo = PROVINCE_REGIONS[r.province];
            if (pInfo?.island !== selectedIsland.value) continue;
        }
        const rid   = r.region_id;
        const score = getScore(r);
        if (!byRegion[rid] || score > getScore(byRegion[rid])) {
            byRegion[rid] = r;
        }
    }
    const sorted = Object.values(byRegion).sort((a, b) => getScore(b) - getScore(a));
    return sorted.map((r, i) => ({ ...r, region_rank: i + 1 }));
});

const myRow    = computed(() => topPerRegion.value.find(r => r.region_id === props.region_id));
const top3     = computed(() => topPerRegion.value.slice(0, 3));
const restList = computed(() => topPerRegion.value.slice(3));

// ── Search ────────────────────────────────────────────────────────────────
const searchTarget = computed(() => {
    const q = searchTerm.value?.trim().toLowerCase();
    if (!q) return null;
    const rows = topPerRegion.value;
    return (
        rows.find(r => r.region_name?.toLowerCase().startsWith(q))?.region_id ??
        rows.find(r => r.region_name?.toLowerCase().includes(q))?.region_id ??
        rows.find(r => r.province?.toLowerCase().includes(q))?.region_id ??
        rows.find(r => (r.director || '').toLowerCase().includes(q))?.region_id ??
        null
    );
});

const rowHighlightClass = (regionId) => {
    const classes = [];
    if (regionId === props.region_id) classes.push('my-region-row');
    if (regionId === searchTarget.value) classes.push('search-highlight');
    return classes.join(' ');
};

const tableRowProps = ({ item }) => ({
    class: [
        item.region_id === props.region_id ? 'my-region-row' : '',
        item.region_id === searchTarget.value ? 'search-highlight' : '',
    ].filter(Boolean).join(' '),
});
...
const chartData = computed(() => {
    const rows = topPerRegion.value;
    return {
        names:     rows.map(r => r.region_name),
        scores:    rows.map(r => getScore(r)),
        provinces: rows.map(r => r.province),
        directors: rows.map(r => r.director || '—'),
        colors:    rows.map(r => r.region_id === props.region_id ? '#6366f1' : '#3b82f6'),
    };
});
    ...
    return {
        ...
        tooltip: {
            ...
            custom: ({ dataPointIndex: i }) => {
                ...
                return `<div style="padding:8px 12px;font-size:12px;font-family:inherit;min-width:220px;">
                    <div style="font-weight:700;margin-bottom:4px;">${data.names[i]}</div>
                    <div style="color:#64748b;margin-bottom:2px;">Top Province: ${data.provinces[i]}</div>
                    <div style="color:#64748b;margin-bottom:6px;">Director: ${data.directors[i]}</div>
                    <div style="display:flex;align-items:center;gap:6px;">
                        <span style="width:10px;height:10px;border-radius:50%;background:${data.colors[i]};flex-shrink:0;"></span>
                        <strong style="color:${data.colors[i]};">${score}% ${label}</strong>
                    </div>
                </div>`;
            },
        },
...
const chartHeight = computed(() => Math.max(320, topPerRegion.value.length * 28 + 40));
</script>
...
<template>
    ...
    <div class="d-flex flex-column gap-3">
        ...
        <!-- My region banner -->
        <v-card v-if="myRow" border elevation="0" rounded="lg" class="my-region-banner">
            <v-card-item class="py-3 px-5">
                <div class="d-flex align-center gap-4 flex-wrap">
                    <v-avatar color="indigo-lighten-5" size="44" rounded="lg">
                        <v-icon :style="{ color: medalColor(myRow.region_rank) }" size="24">mdi-trophy</v-icon>
                    </v-avatar>
                    <div class="flex-1">
                        <div class="text-caption text-medium-emphasis font-weight-bold text-uppercase">
                            {{ region_name }} · Inter-Regional Rank
                        </div>
                        <div class="d-flex align-center gap-2 flex-wrap mt-1">
                            <span class="text-h5 font-weight-black">#{{ myRow.region_rank }}</span>
                            <v-chip size="x-small" :color="bandColor(myRow.adjective_label)" variant="flat" class="text-white font-weight-bold">
                                {{ myRow.adjective_label }}
                            </v-chip>
                            <span class="text-body-2 text-medium-emphasis">{{ getScore(myRow).toFixed(2) }}%</span>
                            <span v-if="selectedTier !== 'all'" class="text-caption text-medium-emphasis">· {{ tierLabel }}</span>
                        </div>
                    </div>
                    <div class="text-right">
                        <div class="text-caption text-medium-emphasis">Top Province</div>
                        <div class="text-body-1 font-weight-bold">{{ myRow.province }}</div>
                        <div class="text-caption text-medium-emphasis">{{ myRow.director || '—' }}</div>
                    </div>
                </div>
            </v-card-item>
        </v-card>
        ...
        <v-card border elevation="0" rounded="lg">
            <div class="px-4 pt-3 pb-2 d-flex align-center justify-space-between gap-2 flex-wrap">
                <div class="d-flex align-center gap-2">
                    ...
                    <span class="text-body-2 font-weight-bold">Top Provincial Director per Region</span>
                    <v-chip size="x-small" color="primary" variant="tonal">{{ topPerRegion.length }} region(s)</v-chip>
                ...
                <v-text-field
                    v-if="viewMode !== 'chart'"
                    v-model="searchTerm"
                    placeholder="Search region, province, or director…"
                    variant="solo-filled"
                    density="compact"
                    hide-details
                    clearable
            ...
            <v-data-table
                v-if="viewMode === 'table'"
                :headers="[
                    { title: 'Rank',        key: 'region_rank', width: '70px',  sortable: true  },
                    { title: 'Region',       key: 'region_name',                 sortable: true  },
                    { title: 'Top Province', key: 'province',                    sortable: true  },
            ...
            >
                <template #item.region_rank="{ item }">
                    <div class="d-flex align-center gap-1">
                        <v-icon v-if="item.region_rank <= 3" size="15" :style="{ color: medalColor(item.region_rank) }">mdi-medal</v-icon>
                        <span v-if="item.region_rank === 1" class="rank-medal rank-gold">1st</span>
                        <span v-else-if="item.region_rank === 2" class="rank-medal rank-silver">2nd</span>
                        <span v-else-if="item.region_rank === 3" class="rank-medal rank-bronze">3rd</span>
                        <span v-else class="text-caption text-medium-emphasis">#{{ item.region_rank }}</span>
                        <v-chip v-if="item.region_id === region_id" size="x-small" color="indigo" variant="tonal" class="ml-1">You</v-chip>
                    </div>
                </template>
                ...
                <template #item.label="{ item }">
                    <v-chip size="x-small" :color="bandColor(item.adjective_label)" variant="tonal" class="font-weight-bold">{{ item.adjective_label }}</v-chip>
                </template>
                <template #no-data>
            ...
            <div v-else-if="viewMode === 'podium'" class="px-5 pt-3 pb-5">
                <div v-if="top3[0]" class="podium-banner mb-4">
                    ...
                    <span class="text-body-2 ml-2">
                        <strong>{{ top3[0].region_name }}</strong> leads {{ selectedYear }}
                        <template v-if="selectedTier !== 'all'"> in {{ tierLabel }}</template>
                        with top province
                        <a :href="`/province-directories/${top3[0].province_url_id}`" class="province-link font-weight-bold">{{ top3[0].province }}</a>
                ...
                <div class="d-flex gap-5">
                    <div class="podium-stage">
                        <div class="podium-items">
                            <div v-if="top3[0]" class="podium-item" :class="rowHighlightClass(top3[0].region_id)" :data-rid="top3[0].region_id">
                                <div class="podium-info">
                                    <v-icon color="amber-darken-1" size="26" class="mb-1">mdi-trophy</v-icon>
                                    <div class="podium-region-name">{{ top3[0].region_name }}</div>
                                    <div class="podium-province-name"><a :href="`/province-directories/${top3[0].province_url_id}`" class="province-link">{{ top3[0].province }}</a></div>
                            ...
                            <div v-if="top3[1]" class="podium-item" :class="rowHighlightClass(top3[1].region_id)" :data-rid="top3[1].region_id">
                                <div class="podium-info">
                                    <div class="podium-region-name">{{ top3[1].region_name }}</div>
                                    <div class="podium-province-name"><a :href="`/province-directories/${top3[1].province_url_id}`" class="province-link">{{ top3[1].province }}</a></div>
                            ...
                            <div v-if="top3[2]" class="podium-item" :class="rowHighlightClass(top3[2].region_id)" :data-rid="top3[2].region_id">
                                <div class="podium-info">
                                    <div class="podium-region-name">{{ top3[2].region_name }}</div>
                                    <div class="podium-province-name"><a :href="`/province-directories/${top3[2].province_url_id}`" class="province-link">{{ top3[2].province }}</a></div>
                        ...
                        <div v-if="top3.length" class="podium-breakdown">
                            <div v-for="p in top3" :key="p.region_id" class="breakdown-col">
                    ...
                    <div class="flex-1 podium-rest-scroll">
                        <div v-for="item in restList" :key="item.region_id" class="podium-rest-row" :class="rowHighlightClass(item.region_id)" :data-rid="item.region_id">
                            <span class="podium-rest-rank">#{{ item.region_rank }}</span>
                            <div class="flex-1" style="min-width:0;">
                                <div class="text-body-2 font-weight-medium text-truncate">{{ item.region_name }}</div>
                                <div class="text-caption text-disabled text-truncate" style="line-height:1.2;">
                        ...
                        <div v-if="!restList.length" class="text-center py-6 text-caption text-medium-emphasis">Only {{ top3.length }} region(s) with data</div>
                    </div>
                </div>
            </div>
            ...
            <div v-else class="px-4 pt-3 pb-5">
                <div class="text-caption text-medium-emphasis mb-3">
                    Score comparison — top province per region · {{ selectedYear }}
                    <template v-if="selectedTier !== 'all'"> · {{ tierLabel }}</template>
                    <template v-if="selectedCategory !== 'overall'"> · {{ selectedCategory }}</template>
                    <span class="ml-2 indigo-chip">Your region is highlighted in indigo</span>
                </div>
                <VueApexCharts
                    v-if="topPerRegion.length"
                    type="bar"
                    :height="chartHeight"
                    :options="chartOptions"
                    :series="chartSeries"
                    :key="`ra-chart-${selectedYear}-${selectedTier}-${selectedCategory}-${selectedIsland}`"
                />
                <div v-else class="text-center py-8 text-medium-emphasis text-body-2">No ranked data found for the selected filters.</div>
            </div>
        </v-card>

        <v-alert v-if="!topPerRegion.length" type="info" variant="tonal" density="compact" class="text-body-2">
            No provinces have submitted KPI data for {{ selectedYear }}
...
.my-region-row td { background: rgba(99, 102, 241, 0.06) !important; }
.my-region-banner { border-left: 4px solid #6366f1 !important; }
...
.podium-province-name { font-size: 13px; color: #64748b; margin-top: 2px; line-height: 1.3; }
...
.indigo-chip { display: inline-block; background: rgba(99,102,241,0.12); color: #4f46e5; border-radius: 4px; padding: 1px 7px; font-size: 12px; font-weight: 600; }
