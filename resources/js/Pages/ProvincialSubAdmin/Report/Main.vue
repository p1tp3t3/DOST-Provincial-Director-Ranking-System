<template>
    <AuthenticatedLayout>
        <div class="grid gap-4 w-full">
            ...
            <v-card class="elevation-1 border-0 rounded-md pa-4" v-if="province?.name">
            ...
            <v-row>
                ...
                <v-col cols="12" md="4">
                    <v-card class="elevation-1 border-0 rounded-md h-100">
                        ...
                        <v-divider></v-divider>
                        <div class="pa-5 d-flex flex-column align-center gap-4" v-if="directorKpi?.total_items > 0">

                            <!-- Rate gauge -->
                            <div class="text-center">
                                <div
                                    class="text-h3 font-weight-bold"
                                    :style="{ color: rateColor(directorKpi.rate) }"
                                >{{ directorKpi.rate }}%</div>
...
const testUserStats = { total: 22, employees: 18, admins: 2, directors: 1, permanent: 10, cos: 5, jo: 3 };
...
const summaryCards = computed(() => [
    ...
    { label: 'Admins',       value: us.value.admins,     color: 'purple',  icon: 'mdi-account-cog-outline'         },
]);
...
const exportPDF = async () => {
    ...
    exportError.value = '';
    try {
        const res  = await fetch('/provincial-sub-admin-report/export', { credentials: 'same-origin' });
        if (!res.ok) throw new Error(`Server error: ${res.status}`);
        const blob = await res.blob();
        const url  = URL.createObjectURL(blob);
        const a    = document.createElement('a');
        a.href     = url;
        a.download = `provincial-report-${new Date().toISOString().slice(0, 10)}.pdf`;
