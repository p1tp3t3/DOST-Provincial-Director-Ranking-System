...
class ProvincialAdminReportController extends Controller
{
    public function index()
    {
        $provinceId = Auth::user()->province_id;

        return inertia('ProvincialAdmin/Report/Main', [
    ...
    public function export()
    {
        ...
        $pdf = Pdf::loadView('reports.provincial-admin-report', [
            'province'      => $this->get_province($provinceId),
            'user_stats'    => $this->get_user_stats($provinceId),
        ...
        return $pdf->download('provincial-admin-report-' . now()->format('Y-m-d') . '.pdf');
    }
    ...
    private function get_user_stats(?int $provinceId): array
    {
        ...
        $profiles = \App\Models\Profile::whereIn('user_id', $empIds)
            ->with('employeeProfile')
            ->get()
            ->keyBy('user_id');

        $byStatus = $employees->groupBy(function ($u) use ($profiles) {
            return $profiles->get($u->id)?->employeeProfile?->status ?? 'unknown';
        })->map->count();
